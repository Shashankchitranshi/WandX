...
(function() {
    ...
    var TradingView = {
        ...
        gId: function() { return "tradingview_" + ((1 + Math.random()) * 1048576 | 0).toString(16).substring(1) },
        onready: function(callback) { if (window.addEventListener) { window.addEventListener("DOMContentLoaded", callback, false) } else { window.attachEvent("onload", callback) } },
        css: function(css_content) {
            var head = document.getElementsByTagName("head")[0];
            var style = document.createElement("style");
            var rules;
            style.type = "text/css";
            if (style.styleSheet) { style.styleSheet.cssText = css_content } else {
                rules = document.createTextNode(css_content);
                style.appendChild(rules)
            }
            head.appendChild(style)
        },
        bindEvent: function(o, ev, fn) { if (o.addEventListener) { o.addEventListener(ev, fn, false) } else if (o.attachEvent) { o.attachEvent("on" + ev, fn) } },
        unbindEvent: function(o, ev, fn) { if (o.removeEventListener) { o.removeEventListener(ev, fn, false) } else if (o.detachEvent) { o.detachEvent("on" + ev, fn) } },
        cloneSimpleObject: function(obj) { if (null == obj || "object" != typeof obj) { return obj } var copy = obj.constructor(); for (var attr in obj) { if (obj.hasOwnProperty(attr)) { copy[attr] = obj[attr] } } return copy },
        isArray: function(obj) { return Object.prototype.toString.call(obj) === "[object Array]" },
        isMobileDevice: function() {
            var ret = { Android: /Android/i.test(navigator.userAgent), BlackBerry: /BlackBerry/i.test(navigator.userAgent), iOS: /iPhone|iPad|iPod/i.test(navigator.userAgent), Opera: /Opera Mini/i.test(navigator.userAgent) };
            ret.any = ret.Android || ret.BlackBerry || ret.iOS || ret.Opera;
            return ret
        }(),
        generateUtmForUrlParams: function(options) { return "utm_source=" + encodeURI(window.location.hostname) + "&utm_medium=widget" + (options.type ? "&utm_campaign=" + options.type : "") + (options.type && options.type === "chart" ? "&utm_term=" + encodeURIComponent(options.symbol) : "") },
        WidgetAbstract: function() {},
        MiniWidget: function(options) {
            this.id = TradingView.gId();
            this.options = { whitelabel: options.whitelabel || "", width: TradingView.WidgetAbstract.prototype.fixSize(options.width) || 300, height: TradingView.WidgetAbstract.prototype.fixSize(options.height) || 400, symbols: options.symbols, tabs: options.tabs || "", symbols_description: options.symbols_description || "", customer: options.customer || "", container: options.container_id || "", greyText: options.greyText || "", large_chart_url: options.large_chart_url || "", large_chart_target: options.large_chart_target || "", gridLineColor: options.gridLineColor || "", fontColor: options.fontColor || "", underLineColor: options.underLineColor || "", trendLineColor: options.trendLineColor || "", timeAxisBackgroundColor: options.timeAxisBackgroundColor || "", activeTickerBackgroundColor: options.activeTickerBackgroundColor || "", noGraph: options.noGraph || false, locale: options.locale, styleTickerActiveBg: options.styleTickerActiveBg || "", styleTabActiveBorderColor: options.styleTabActiveBorderColor || "", styleTickerBodyFontSize: options.styleTickerBodyFontSize || "", styleTickerBodyFontWeight: options.styleTickerBodyFontWeight || "", styleTickerHeadFontSize: options.styleTickerHeadFontSize || "", styleTickerHeadFontWeight: options.styleTickerHeadFontWeight || "", styleTickerChangeDownColor: options.styleTickerChangeDownColor || "", styleTickerChangeUpColor: options.styleTickerChangeUpColor || "", styleTickerLastDownBg: options.styleTickerLastDownBg || "", styleTickerLastUpBg: options.styleTickerLastUpBg || "", styleTickerSymbolColor: options.styleTickerSymbolColor || "", styleTickerSymbolHoverTextDecoration: options.styleTickerSymbolHoverTextDecoration || "", styleTickerActiveSymbolTextDecoration: options.styleTickerActiveSymbolTextDecoration || "", styleTabsActiveBorderColor: options.styleTabsActiveBorderColor || "", styleTabsNoBorder: options.styleTabsNoBorder || "", styleWidgetNoBorder: options.styleWidgetNoBorder || "" };
            this.createWidget()
        },
        MediumWidget: function(options) {
            this.id = TradingView.gId();
            this.options = { container: options.container_id || "", width: TradingView.WidgetAbstract.prototype.fixSize(options.width) || "", height: TradingView.WidgetAbstract.prototype.fixSize(options.height) || "", symbols: options.symbols, greyText: options.greyText || "", symbols_description: options.symbols_description || "", large_chart_url: options.large_chart_url || "", customer: options.customer || "", gridLineColor: options.gridLineColor || "", fontColor: options.fontColor || "", underLineColor: options.underLineColor || "", trendLineColor: options.trendLineColor || "", timeAxisBackgroundColor: options.timeAxisBackgroundColor || "", chartOnly: !!options.chartOnly, locale: options.locale, whitelabel: !!options.whitelabel || "" };
            this.createWidget()
        },
        widget: function(options) {
            ...
            if (options.news && options.news.length) {
                ...
                if (!this.options.news_vendors) { delete this.options.news_vendors }
            }
            if (isFinite(options.widgetbar_width) && options.widgetbar_width > 0) { this.options.widgetbar_width = options.widgetbar_width }
        ...
        UserInfoWidget: function(options) {
            this.options = { container: options.container_id || "", width: options.width || 1040, height: options.height || 340, username: options.username || "", locale: options.locale, type: "user-info" };
            this.createWidget(options)
        },
        QuotesProvider: function(options) {
            ...
            function resolveSymbol(symbolObj) {
                ...
                if (success) { postMessage.on("success" + symbol, function(data) { success(data) }) }
                if (error) { postMessage.on("error" + symbol, function(data) { error(data) }) }
            }
        }
    };
    ...
    TradingView.DependenciesManager.prototype._loaded = function(script) {
        for (var i in this.scripts[script].callbacks) { this.scripts[script].callbacks[i]() }
    ...
    TradingView.DependenciesManager.prototype.depends = function(scripts, cbid, cb) {
        for (var i = 0; i < scripts.length; i++) {
            if (this.scripts[scripts[i]] && this.scripts[scripts[i]].status === true) { cb() } else if (this.scripts[scripts[i]] && this.scripts[scripts[i]].status === false) { this.scripts[scripts[i]].callbacks[cbid] = cb } else {
    ...
    TradingView.ChatWidgetEmbed.prototype = {
        ...
        widgetCode: function() { var room = this.options.room ? "#" + encodeURIComponent(this.options.room) : ""; var query = "&" + TradingView.createUrlParams({ whotrades: this.options.whotrades, locale: this.options.locale }); var utm = TradingView.generateUtmForUrlParams(this.options, { type: "UserInfoWidget" }); var widget_url = TradingView.chatHost + "/chatwidgetembed/?" + utm + query + room; return "<iframe" + ' src="' + widget_url + '"' + (this.options.width ? ' width="' + this.options.width + '"' : "") + (this.options.height ? ' height="' + this.options.height + '"' : "") + ' frameborder="0" allowTransparency="true" scrolling="no"></iframe>' }
    };
    TradingView.IdeaWidget.prototype = {
        createWidget: function() {
            ...
            this.postMessage.on("resize", function(data) { if (data.id === self.id) { c.style.height = data.height + "px" } }, true)
        },
        widgetCode: function() {
            var query = TradingView.createUrlParams({ id: this.id, width: this.options.width, height: this.options.height, idea: this.options.idea, profile_url: this.options.userProfileUrl, chart_url: this.options.chartUrl, whotrades: this.options.whotrades, locale: this.options.locale });
        ...
        getSymbol: function(callback) { this.postMessage.on("symbolInfo", callback) }
    };
    ...
    TradingView.IdeasStreamWidget.prototype = {
        ...
        widgetCode: function() { var options = this.options; var query = TradingView.createUrlParams({ id: this.id, width: options.width, height: options.height, symbol: options.symbol, username: options.username, mode: options.mode, profile_url: options.userProfileUrl, idea_url: options.ideaUrl, publish_source: options.publishSource, sort: options.sort, stream: options.stream, interval: options.interval, time: options.time, wait_symbol: options.waitSymbol, request_host: TradingView.ideasHost, hide_desc: options.hideDescription, s_count: options.startingCount, bg_color: options.bgColor, h_color: options.headerColor, borderColor: options.borderColor, locale: options.locale }); var utm = TradingView.generateUtmForUrlParams(this.options) + "&"; var widget_url = TradingView.host + "/ideaswidgetembed/?" + utm + query; return '<iframe id="' + this.id + '"' + ' src="' + widget_url + '"' + ' width="' + options.width + '"' + (options.height ? ' height="' + options.height + '"' : "") + ' frameborder="0" allowTransparency="true" scrolling="no"></iframe>' },
    ...
    TradingView.MiniWidget.prototype = {
        createWidget: function() {
            ...
            if (!options.noLogoOverlay && !options.whitelabel) {
                ...
                widget_code = TradingView.WidgetAbstract.prototype.addFooterLogo.call(this, widget_code, { greyText: greyText, linkText: "TradingView" })
            }
        ...
        widgetCode: function() {
            ...
            var prepareSymbols = function(symbols) {
                ...
                for (var i = 0; i < symbols.length; i++) {
                    ...
                    } else if (typeof _s === "string") { _symbols.push(encodeURIComponent(_s)) }
                }
                return _symbols.join(",")
            };
            if (this.options.tabs) {
                for (var i = 0, tl = this.options.tabs.length; i < tl; i++) { var tab = this.options.tabs[i]; if (this.options.symbols[tab]) { symbols += (symbols ? "&" : "") + encodeURIComponent(tab) + "=" + prepareSymbols(this.options.symbols[tab]) } }
                tabs = "&tabs=" + encodeURIComponent(this.options.tabs.join(","))
            } else { if (this.options.symbols) { symbols = "symbols=" + prepareSymbols(this.options.symbols) } }
            if (this.options.symbols_description) { for (var key in this.options.symbols_description) { symbols_description += "&" + encodeURIComponent(key) + "=" + encodeURIComponent(this.options.symbols_description[key]) } }
            if (this.options.customer) { site_path = "/" + this.options.customer + site_path }
    ...
    TradingView.widget.prototype = {
        create: function() {
            ...
            var self = this;
            var c;
            if (!this.options.noLogoOverlay) { widget_html = TradingView.WidgetAbstract.prototype.addWrapperFrame.call(this, widget_html) }
            addWidget(widget_html, this.options.container);
            ...
            TradingView.bindEvent(c, "load", function() {
                self.postMessage.get("widgetReady", {}, function() {
                    var i;
                    self._ready = true;
                    for (i = self._ready_handlers.length; i--;) { self._ready_handlers[i].call(self) }
                })
            });
            ...
            self.postMessage.on("openChartInPopup", function(data) {
                ...
                for (var i = chartProps.length - 1; i >= 0; i--) { var name = chartProps[i]; var value = data[name]; if (value) { urlOptions[name] = value } }
        ...
        generateUrl: function(options) {
            ...
            var site_path;
            if (options.customer === "cme") { site_path = "/cmewidgetembed/" } else if (options.customer) { site_path = "/" + options.customer + "/widgetembed/" } else { site_path = "/widgetembed/" }
            ...
            function studiesArg() { if (!options.studies || !TradingView.isArray(options.studies)) { return "" } if (typeof options.studies[0] === "string") { return arg("studies", encodeURIComponent(options.studies.join(""))) } return jsonArg("studies", options.studies) }
    ...
    TradingView.chart.prototype = {
        create: function() {
            ...
            TradingView.onready(function() {
                var rf = false;
                if (document.querySelector && document.querySelector('a[href$="/v/' + self.options.chart + '/"]')) { rf = true }
                if (!rf) { var anchors = document.getElementsByTagName("a"); var re_short = new RegExp("/v/" + self.options.chart + "/$"); var re_full = new RegExp("/chart/([0-9a-zA-Z:+*-/()]+)/" + self.options.chart); for (var i = 0; i < anchors.length; i++) { if (re_short.test(anchors[i].href) || re_full.test(anchors[i].href)) { rf = true; break } } }
                if (rf) {
                    c.src += "#nolinks";
                    c.name = "nolinks"
                }
        ...
        getKeyHandler: function() { var that = this; if (!this.keyHandler) { this.keyHandler = function(e) { if (e.keyCode === 27) { that.toggleFullscreen() } } } return this.keyHandler },
        getSymbolInfo: function(callback) { this.postMessage.get("symbolInfo", {}, callback) }
    };
        ...
        TradingView.dependenciesManager.depends(["/static/lib/external/spin.min.js"], "authWidget", function() {
            ...
            var onClose = function(data) {
                if (data.id === id) {
                    ...
                    if (data.user) { callback(data.user) }
    ...
    TradingView.onLoginStateChange = function(options, callback) {
        ...
        var c = TradingView.gEl(id);
        var postMessage = TradingView.postMessageWrapper(c.contentWindow, id);
        var onLoginStateChange = function(data) { if (data.id === id) { callback(data.user) } };
    ...
    TradingView.postMessageWrapper = function() {
        ...
        if (window.addEventListener) {
            window.addEventListener("message", function(e) {
                var msg;
                try { msg = JSON.parse(e.data) } catch (error) { return }
                if (!msg.provider || msg.provider !== provider_id) { return }
                if (msg.type === "get") {
                    if (!on_handlers[msg.name]) { return }
                    on_handlers[msg.name].forEach(function(on_handler) {
                        if (typeof on_handler === "function") {
                            on_handler.call(msg, msg.data, function(result) {
                                var reply = { id: msg.id, type: "on", name: msg.name, client_id: msg.client_id, data: result, provider: provider_id };
                    ...
                    if (!on_handlers[msg.name]) { return }
                    on_handlers[msg.name].forEach(function(on_handler) { if (typeof on_handler === "function") { on_handler.call(msg, msg.data, function() {}) } })
                }
            })
        }
        return function(target, client_id) {
            ...
            return {
                on: function(name, callback, keepPrevious) {
                    if (!on_handlers[name] || !keepPrevious) { on_handlers[name] = [] }
                    on_handlers[name].push(callback)
                },
                off: function(name, callback) { if (!on_handlers[name]) { return false } var index = on_handlers[name].indexOf(callback); if (index > -1) { on_handlers[name].splice(index, 1) } },
                ...
                post: function(target, name, data) { var msg = { id: post_id++, type: "post", name: name, data: data, provider: provider_id }; if (target && typeof target.postMessage === "function") { target.postMessage(JSON.stringify(msg), "*") } }
            }
        }
    }();
    ...
    TradingView.WidgetPopup.prototype = {
        create: function() {
            ...
            if (typeof this.popupWindow === "function") { this.popupWindow.focus() }
        },
